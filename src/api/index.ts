...
export const fetchUserData =  async (e,n) =>  {
    ...
    try {
        let response = await axios.get('/api/getmeetingroombypage', {
            params: {
                page: e,
                size: 10,
                // room_id:'',
...
export const DeleteData = async (e) => {
  ...
  try {
    ...
    // 发起 POST 请求
    const response = await fetch('/api/delmeetingroom', {
...
export const fetchUserData2 =  async (e,n) =>  {
    ...
    try {
        let response = await axios.get('/api/getuserbypage', {
            params: {
                page: e,
                size: 10,
                // room_id:'',
...
export const fetchUserData3 =  async (e,n) =>  {
    ...
    try {
        let response = await axios.get('/api/getreservationbypage', {
            params: {
                page: e,
                size: 10,
                // room_id:'',
        ...
        ans.forEach(element => {
            element.time = element.start_time.substring(0, 5) + ' - ' + element.end_time.substring(0, 5);
            if(element.deletedAt==null){
                element.status = true 
            }
            else{
...
export const DeleteData3 = async (e) => {
    ...
    try {
      ...
      // 发起 POST 请求
      const response = await fetch('/api/delreservation', {
          method: 'DELETE', // 指定请求方法为 POST
          headers: {
              'Content-Type': 'application/json', // 设置请求头，告诉服务器发送的是 JSON 数据
              // 根据需要可能还需要添加其他头部信息，如认证令牌等
